class Card {
    ...
    like = (event) => {
        ...
        if (this.likes.filter(like => like._id == myId ).length > 0) {
            const del = getsCardsOptions(`cards/like/${this.idCard}`, 'DELETE');
            ...
            const put = getsCardsOptions(`cards/like/${this.idCard}`, 'PUT');
    ...
    remove = (event) => {
        ...
        if (window.confirm("Вы действительно хотите удалить эту карточку?")) { 
            const delCard = getsCardsOptions(`cards/${this.idCard}`, 'DELETE');
    ...
    create() {
        ...
        if (this.owner._id == myId){
            ...
            delIcon.addEventListener('click', this.remove);
    ...
    addListeners(placeCard) {
        ...
        placeCard.querySelector('.place-card__image').addEventListener('click', this.open);
    } 
}